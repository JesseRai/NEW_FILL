...
CTE_MONTH_CAT_SHARE AS (
    SELECT
        ...
        (mc.category_month_revenue::NUMERIC / NULLIF(mt.month_total_revenue,0))::NUMERIC(12,4) AS category_share_of_month
...
CTE_CUSTOMER_RANK AS (
    SELECT 
        cr.*, 
        RANK() OVER (PARTITION BY paid_month, category_name ORDER BY customer_month_revenue DESC, customer_id) AS customer_rank_in_category_month
